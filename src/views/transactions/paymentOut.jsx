...
const validationSchema = Yup.object().shape({
  ...
  amountGiven: Yup.number().required('Amount Recieved is required').positive('Amount Recieved must be positive'),
  paymentMode: Yup.string().required('Payment Mode is required')
});
const paymentIn = () => {
  const [isLoading, setIsLoading] = useState(false);
  const [partyNames, setPartyNames] = useState([]);
  const { project } = React.useContext(AuthContext);
    ...
    <div>
      <Formik
        ...
        onSubmit={(values, { setSubmitting, resetForm }) => {
          console.log(values);
          setIsLoading(true);
          BaseURL.post('api/add/paymentout', { ...values, projectId: project, createdBy: 1 })
            .then((res) => {
              console.log(res);
              if (res.data) {
                Swal.fire({
                  title: 'Payment Out Added Successfully',
                  icon: 'success',
            ...
            <React.Fragment>
              ...
              <Row>
                <Col>
                  <Card>
                    <Card.Header>
                      <Card.Title as="h5">Payment Out</Card.Title>
                    </Card.Header>
                    <Card.Body>
                      <Grid container spacing={2}>
                        <Grid item xs={12} sm={12}>
                          <Typography variant="h5">Enter Payment Out Details</Typography>
                        </Grid>
                        ...
                        <Grid item xs={12} sm={6}>
                          <FormControl fullWidth size="small" error={Boolean(touched.categoryId && errors.categoryId)}>
                            <InputLabel required>Payment To</InputLabel>
                            <Select
                              label="partyName"
                              id="partyName"
                              name="partyName"
                        ...
                        <Grid item xs={12} sm={6}>
                          <TextField
                            ...
                            variant="outlined"
                            required
                            error={Boolean(touched.amountGiven && errors.amountGiven)}
                            helperText={touched.amountGiven && errors.amountGiven}
                          />
                        </Grid>
                        <Grid item xs={12} sm={6}>
                          <TextField
